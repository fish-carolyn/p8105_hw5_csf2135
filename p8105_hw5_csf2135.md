...
``` r
MD_unsolved_homicides <- df_city_state_unsolved  |> 
                                filter(city_state == "Baltimore, MD")  |> 
                                pull(unsolved_homicides)

MD_total_homicides <- df_city_state_unsolved  |> 
                                filter(city_state == "Baltimore, MD")  |> 
                                pull(total_homicides)
                                

MD_homicide_prop <- prop.test(MD_unsolved_homicides, MD_total_homicides) |> 
  broom::tidy() |>
  select(estimate, `conf.high`, `conf.low`) 

knitr::kable(MD_homicide_prop, format = "markdown")
```

|  estimate | conf.high |  conf.low |
|----------:|----------:|----------:|
| 0.6455607 | 0.6631599 | 0.6275625 |
